...
def run_EX(loader_test, config):
    ...
    df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_exact_{num_var}-{num_ineq}.csv")
...
def run_RR(loader_test, config):
    ...
    os.makedirs("result/sol", exist_ok=True)
    os.makedirs("result/stat", exist_ok=True)
    df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_rel_{num_var}-{num_ineq}.csv")
...
def run_N1(loader_test, config):
    ...
    os.makedirs("result/sol", exist_ok=True)
    os.makedirs("result/stat", exist_ok=True)
    df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_root_{num_var}-{num_ineq}.csv")
...
def run_AS(loader_train, loader_test, loader_val, config):
    ...
    os.makedirs("result/stat", exist_ok=True)
    if config.project:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_cls{penalty_weight}_{num_var}-{num_ineq}-p.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_cls{penalty_weight}_{num_var}-{num_ineq}-p.csv")
    else:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_cls{penalty_weight}_{num_var}-{num_ineq}.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_cls{penalty_weight}_{num_var}-{num_ineq}.csv")
...
def run_DT(loader_train, loader_test, loader_val, config):
    ...
    os.makedirs("result/stat", exist_ok=True)
    if config.project:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_thd{penalty_weight}_{num_var}-{num_ineq}-p.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_thd{penalty_weight}_{num_var}-{num_ineq}-p.csv")
    else:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_thd{penalty_weight}_{num_var}-{num_ineq}.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_thd{penalty_weight}_{num_var}-{num_ineq}.csv")
...
def run_RS(loader_train, loader_test, loader_val, config):
    ...
    os.makedirs("result/stat", exist_ok=True)
    if config.project:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_ste{penalty_weight}_{num_var}-{num_ineq}-p.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_ste{penalty_weight}_{num_var}-{num_ineq}-p.csv")
    else:
        df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_ste{penalty_weight}_{num_var}-{num_ineq}.csv")
        df[["Obj Val", "Mean Violation", "Max Violation", "Num Violations", "Elapsed Time"]].to_csv(f"result/stat/nc_ste{penalty_weight}_{num_var}-{num_ineq}.csv")
...
def run_LR(loader_train, loader_test, loader_val, config):
    ...
    print(df.describe())
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    df[["Param", "Sol", "Viol"]].to_csv(f"result/sol/nc_lrn{config.penalty}_{num_var}-{num_ineq}.csv")
