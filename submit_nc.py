...
for size in sizes:
    ...
    for penalty in penalty_weights:
        ...
        submit_job(experiments.nonconvex.run_AS, loader_train, loader_test, loader_val, config,
                   timeout_min=timeout,
                   csv_path=f"result/stat/nc_cls{penalty}_{size}-{size}.csv")
        ...
        submit_job(experiments.nonconvex.run_AS, loader_train, loader_test, loader_val, config,
                   timeout_min=timeout,
                   csv_path=f"result/stat/nc_cls{penalty}_{size}-{size}-p.csv")
        # Dynamic threshold rounding + projection
        print("        Dynamic Threshold, with projection")
        submit_job(experiments.nonconvex.run_DT, loader_train, loader_test, loader_val, config,
                   timeout_min=timeout,
                   csv_path=f"result/stat/nc_thd{penalty}_{size}-{size}-p.csv")
        # STE rounding + projection
        print("        STE Rounding, with projection")
        submit_job(experiments.nonconvex.run_RS, loader_train, loader_test, loader_val, config,
                   timeout_min=timeout,
                   csv_path=f"result/stat/nc_ste{penalty}_{size}-{size}-p.csv")
        print()
